...
As a measure of the number of erroneous peak predictions, we calculated the $\mathit{FDR}=\mathit{FP}/(\mathit{FP}+\mathit{TP})$ and $\mathit{FNR}=\mathit{FN}/(\mathit{FN}+\mathit{TP})$ for each predicted spectrum. Here $\mathit{FP}$ is the number of peaks predicted in excess to be present in a spectrum that was absent in the observed spectrum, $\mathit{FN}$ is the number of peaks deficently predicted to be absent in a spectrum that was present in the observed spectrum, and $\mathit{TP}$ is the number of peaks accurately predicted to be present in a spectrum that was present in the observed spectrum.

\subsection*{Post-processing of Predicted intensities}
For the final result, we use the same post-processing on the predicted spectrum used in Prosit ​\cite{Gessulat2019-el}. That is, we set ions with a predicted negative intensity to zero, i.e., a negative intensity indicates an absent peak. Furthermore, we set all ion's intensity that's not obtainable for any given peptide due to too low a charge state or too low peptide length to -1, however, such peaks were excluded from similarity measurements.
...
A possible interpretation of the fact that among the combinations of predictiors with RNNs and Transformers as encoders and decoers indicate better performance for predictors with Transformers as decoders could be that the Transformer architecture is better in utilizing the metadata, i.e. the collision energy and charge state information. A future direction of the project could be to investigate the source of the improved accuracy by removing this information from the different decoders. 
